...
traverseTriangleRaw triangle [] point = 0
traverseTriangleRaw triangle (x:xs) point
  | currentelem == Nothing = 0
  | otherwise = (fromJust currentelem) + (traverseTriangleRaw triangle xs (moveDirection x point))
  where currentelem = getElem triangle point
...
main = greatest $ traverseTriangleAll problemTriangle $ genPaths problemTriangle